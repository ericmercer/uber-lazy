...
single representation. These sets, sometimes known as value sets or
value summaries, allow multiple heaps to be represented simultaneously
...
recursive definition of objects~\cite{..}. Also, it is not obvious how
to support heap updates in value sets, and in particular, on-the-fly
lazy initialization as is common in GSE, so it is not unusual to
require a fully initialized set of input heaps in the analysis. As is
...
and when heap updates are supported, with or without over
approximation, the path constraint must be rewritten to reflect the
update, and the constraints in the value set must also be rewritten.
...
to several different locations; each edge is predicated on constraints
...
Aside from the novelty of the heap summary and it application to
symbolic execution, the new data structure appears to ameliorate state
explosion in symbolic execution. A preliminary implementation in Java
...
\noindent{The rest of this paper discusses a formal language semantics
  for the summary heap algorithm definition with the definition of the
  summary heap. The paper then presents the application of summary
  heaps to symbolic execution. Following the algorithm definition, the
  existence proof of bisimulation is given with the accompanying
  corollaries for soundness and completeness. This is followed by the
  implementation in JPF and the evaluation. The paper ends with the
