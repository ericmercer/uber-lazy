...
%\and
	\inferrule[Lazy Initialization]{
	  \Lambda = \{ l \mid \exists \phi\ ((\phi\ l) \in L(r) \wedge  R(l,f) = \emptyset\}\\
      \Lambda \neq \emptyset \\
      l_x = \mathrm{min}_l(\Lambda) \\\\
      \mathrm{type}(\cfgnt{f}) = \mathrm{C}_r\\
      \mathrm{init}_r() = r_f\\ 
      \mathrm{init}_l(\mathrm{C}_r) = l_f \\\\
      \rho = \{ (r^\prime\ \phi^\prime\ l^\prime) \mid \mathrm{isInit}(r^\prime) \wedge (\phi^\prime\ l^\prime) \in L(r^\prime) \wedge \mathrm{type}(l^\prime) = \mathrm{C}_r \} \\\\
      \theta_\mathit{alias} = \{ (\phi\ l) \mid (r^\prime\ \phi^\prime\ \l) \in \rho \wedge \phi = (\phi^\prime \wedge r_f \neq r_\mathit{null} \wedge r_f = r^\prime \wedge (\wedge_{(r^{\prime\prime}\ \phi^{\prime\prime}\ l^{\prime\prime}) \in \rho\ (r^{\prime\prime} \neq r^\prime)} r_f \neq r^{\prime\prime} )) \} \\\\
      \theta_\mathit{new} = \{(\phi\ l_f) \mid \phi = (r_f \neq r_\mathit{null} \wedge (\wedge_{(r^\prime,\ \phi^\prime,\ l^\prime) \in \rho} r_f \ne r^\prime))\}\\\\
      \theta_\mathit{null} = \{ (\phi\ l_\mathit{null}) \mid \phi = (r_f = r_\mathit{null}) \}\\\\
...
\and
	\inferrule[New]{
      ...
      R^\prime = R[\forall \cfgnt{f} \in \mathit{fields}(\mathrm{C})\ ((l\ \cfgnt{f}) \mapsto \mathrm{fresh}_r())] \\\\
      \rho = \{r^\prime \mid \exists \cfgnt{f} \in \mathit{fields}(\mathrm{C})\ (r^\prime = R^\prime(l, f))\}\\\\
      L^\prime = L[r \mapsto \{(\phi_T\ l)\} \wedge \forall r^\prime \in \rho\ (r^\prime \mapsto (\phi_T\ l_\mathit{null}))]
    }{
...
\and
	\inferrule[Field Access]{
      \forall (\phi\ l) \in L(r)\ (l = l_{\mathit{null}} \rightarrow \neg \mathbb{S}(\phi \wedge g)) \\
      \mathrm{fresh}_r() = r_f
    }{
      (L\ R\ g\ \eta\ r\ (*\ \$\ \cfgnt{f} \rightarrow k)) \rightarrow 
...
\and
    ...
    \cfgnt{v}_1 \not\in \{\cfgt{true}, \cfgt{false}\}\\\\
    \theta_\alpha = \{\phi_0 \wedge \phi_1 \mid \exists l\ ((\phi_0\ l) \in L(v_0) \wedge (\phi_1\ l) \in L(v_1))\} \\\\
    \theta_0 = \{\phi_0 \mid \exists l_0\ ((\phi_0\ l_0) \in L(v_0) \wedge \forall (\phi_1\ l_1) \in L(v_1)\ (l_0 \neq l_1))\} \\\\
    \theta_1 = \{\phi_1 \mid \exists l_1\ ((\phi_1\ l_1) \in L(v_1) \wedge \forall (\phi_0\ l_0) \in L(v_0)\ (l_0 \neq l_1))\} \\\\
    ...
    (L\ R\ g\ \eta\ \cfgnt{v}_0\ (\cfgnt{v}_1\; \cfgt{=}\; \cfgt{*} \rightarrow \cfgnt{k})) \rightarrow
    (L\ R\ g^\prime\ \eta\ \cfgt{true}\ \cfgnt{k})
    }
\and
    \inferrule[Equals (references-false)]{
    \cfgnt{v}_0 \not\in \{\cfgt{true}, \cfgt{false}\} \\
    \cfgnt{v}_1 \not\in \{\cfgt{true}, \cfgt{false}\}\\\\
    \theta_\alpha = \{\phi_0 \rightarrow \neg \phi_1 \mid \exists l\ ((\phi_0\ l) \in L(v_0) \wedge (\phi_1\ l) \in L(v_1))\} \\\\
    \theta_0 = \{\phi_0 \mid \exists l_0\ ((\phi_0\ l_0) \in L(v_0) \wedge \forall (\phi_1\ l_1) \in L(v_1)\ (l_0 \neq l_1))\} \\\\
    \theta_1 = \{\phi_1 \mid \exists l_1\ ((\phi_1\ l_1) \in L(v_1) \wedge \forall (\phi_0\ l_0) \in L(v_0)\ (l_0 \neq l_1))\} \\\\
...
\and
	...
	r_x = \eta(\cfgnt{x})\\
\forall (\phi\ l) \in L(r_x)\ (l = l_{\mathit{null}} \rightarrow \neg \mathbb{S}(\phi \wedge g)) \\\\
      \Psi_x =\{(r_\mathit{old}\ \phi\ l) \mid (\phi\ \cfgnt{l}) \in L(r_x) \wedge r_\mathit{old} \in R(l,\cfgnt{f}) \}\\\\
      X = \{ (r_\mathit{old}\ \theta\ l) \mid \exists \phi\ ((r_\mathit{old}\ \phi\ l)\in \Psi_x \wedge \theta = \mathbb{ST}(L,\cfgnt{r},\phi) \cup \mathbb{ST}(L,r_\mathit{old},\neg\phi)) \}\\\\
      R^\prime = R[\forall (r_\mathit{old}\ \theta\ l^\prime) \in X\ ((l\ f) \mapsto \mathrm{fresh}_r())]\\\\
      L^\prime = L[\forall (r_\mathit{old}\ \theta\ l^\prime) \in X\ (\exists r^{\prime} = R^\prime(l,f)\ ( r^{\prime} \mapsto \theta) )]
    }{
