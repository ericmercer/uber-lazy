...
\and
    ...
    \in \theta_2} \neg \phi_2) \\\\
      ...
      X = \mathbb{B}(l, \phi_g) \\ \mathbb{C}(L^\prime, R, X, \eta, k) = 1}{
    (L\ R\ \eta\ \cfgnt{v}_0\ (\cfgnt{v}_1\; \cfgt{=}\; \cfgt{*} \rightarrow \cfgnt{k})) \rightarrow
...
\and
	\inferrule[Field Write]{
      L(r) \neq \emptyset \\
      \Psi_x =\{(\cfgnt{x}^\prime,\phi)\ | \exists(\cfgnt{l},\phi) \in L(\cfgnt{x}) \wedge \exists \cfgnt{x}^\prime \in R(\cfgnt{l,f}) \}\\\\
      L^\prime = [ \cfgnt{x}_i \mapsto ((L(\cfgnt{r} ) \wedge \phi_i) \cup ( L(\cfgnt{x}_i) \wedge \neg \phi_i) | (\cfgnt{x}_i,\phi_i) \in \Psi_x]
    }{
      (L\ R\ \eta\ \cfgnt{r}\ (\cfgnt{x}\ \cfgt{\$}\ \cfgnt{f}\ \cfgt{:=}\ \cfgnt{*})\ \rightarrow\ k) \rightarrow 
      (L^\prime R\ \eta\ k))
	}	
