...
being location mutual exclusive. The $S$-relation effectively produces
...
\begin{lemma}[$\rightarrow^*_S$ preserves location mutual exclusion]
\label{lem:S-mutex}
...
\begin{lemma}[$\rightarrow_\mathit{FA}$ preserves location mutual exclusion]
\label{lem:FA-mutex}
Given a summary state, $s_\varsigma$, with a location mutual exclusive
heap, $(\cfgnt{L}_\varsigma\ \cfgnt{R}_\varsigma) =
\mathrm{heap}(s_\varsigma)$, the new heap,
$(\cfgnt{L}_\varsigma^\prime\ \cfgnt{R}_\varsigma^\prime) =
\mathrm{heap}(s_\varsigma^\prime)$, in any related state on the field
access rule, $s_\varsigma \rightarrow_\mathit{FA} s_\varsigma^\prime$, is also location mutual exclusive.
\end{lemma}
\begin{proof}
...
$S$-relation is location mutual exclusive, so it is only needed to
show that location mutual exclusion is preserved by the call to the value function
$\mathbb{VS}$ in the rule.

\defref{def:VS} is the value function definition. From the definition,
each constraint in each member of the value set has the form
$(\phi\wedge \phi^\prime\ l)$ where $\phi$ and $\phi^\prime$ are the
access constraints. The first constraint $\phi$ represents the
condition under which the location is the location referenced by
$r$. The second constraint, predicated on having resolved the actual
location $\cfgnt{l}$ (expressed in $\phi$), is the condition under
which the location $\cfgnt{l}^\prime$ is the actual object for the
field as the reference for that field, $\cfgnt{r}^\prime =
\cfgnt{R}(\cfgnt{l},\cfgnt{f})$, may point to several different
locations.

Choose any two distinct members of the value set, $(\phi_\alpha \wedge
\phi_\alpha^\prime\ \ l_\alpha)$ and $(\phi_\beta \wedge
\phi_\beta^\prime\ \ l_\beta)$. 
\begin{itemize}
\item If $\phi_\alpha = \phi_\beta$, then $\phi_\alpha^\prime \wedge
  \phi_\beta^\prime = \cfgt{false}$ because $\cfgnt{l}_\alpha$ and
  $\cfgnt{l}_\beta$ come from the same field reference, and since the
  heap, $(\cfgnt{L}\ \cfgnt{R})$, is location mutual exclusive, locations
  under a common reference must be mutual exclusive.
\item If $\phi_\alpha \ne \phi_\beta$, then $\phi_\alpha \wedge \phi_\beta = \cfgt{false}$ because $\phi_\alpha$ comes from a location connected to $r$ that is different that the location connected to $r$ for $\phi_\beta$, and since the
  heap, $(\cfgnt{L}\ \cfgnt{R})$, is location mutual exclusive, locations
  under a common reference must be mutual exclusive.
\end{itemize}
The only change to the heap after the $S$-relation is applied is the addition of the new reference $\cfgnt{r}^\prime$ mapping to the value set from the value function. Since that set is location mutual exclusive and connect to $r^\prime$, the new heap $(\cfgnt{L}_\varsigma^\prime\ \cfgnt{R}_\varsigma^\prime) =
\mathrm{heap}(s_\varsigma^\prime)$ is also location mutual exclusive.
\end{proof}
