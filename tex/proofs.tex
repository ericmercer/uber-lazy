...
$(\cfgnt{L}_\varsigma^\prime\ \cfgnt{R}_\varsigma^\prime) =
\mathrm{heap}(s_\varsigma^\prime)$, in any state related by the field
write rule, $s_\varsigma \rightarrow_\mathit{FW} s_\varsigma^\prime$,
is also deterministic.
\end{lemma}
\begin{proof}
...
$\cfgnt{L}_\varsigma(\cfgnt{r})$ on the same reference in a
deterministic heap $(\cfgnt{L}_\varsigma\ \cfgnt{R}_\varsigma)$; thus,
...
$(\cfgnt{L}_\varsigma\ \cfgnt{R}_\varsigma)$ and meet the criteria so
that $(\cfgnt{L}_\varsigma^\prime\ \cfgnt{R}_\varsigma^\prime)$ is also
deterministic.
\end{proof}

\begin{lemma}[$\rightarrow_J$ preserves heap determinism]
\label{lem:J-determ}
Given a state, $s_\varsigma$, with a deterministic heap,
$(\cfgnt{L}_\varsigma\ \cfgnt{R}_\varsigma) =
\mathrm{heap}(s_\varsigma)$, the new heap,
$(\cfgnt{L}_\varsigma^\prime\ \cfgnt{R}_\varsigma^\prime) =
\mathrm{heap}(s_\varsigma^\prime)$, in any state related by the
Javalite relation, $s_\varsigma \rightarrow_J s_\varsigma^\prime$, is
also deterministic.
\end{lemma}
\begin{proof}
Proof by definition of $\rightarrow_J$ in \figref{fig:javalite-common}.

Every rule in $\rightarrow_J$ except \texttt{New} leaves the heap
...
$(\cfgnt{L}_\varsigma^\prime\ \cfgnt{R}_\varsigma^\prime)$ is also
deterministic.
\end{proof}

\begin{theorem}[$\rightarrow_\varsigma$ preserves heap determinism]
\label{thm:determ}
Given a state, $s_\varsigma$, with a deterministic heap,
$(\cfgnt{L}_\varsigma\ \cfgnt{R}_\varsigma) =
\mathrm{heap}(s_\varsigma)$, the new heap,
$(\cfgnt{L}_\varsigma^\prime\ \cfgnt{R}_\varsigma^\prime) =
\mathrm{heap}(s_\varsigma^\prime)$, in any state related by the
symbolic relation, $s_\varsigma \rightarrow_\varsigma s_\varsigma^\prime$, is
also deterministic.
\end{theorem}
\begin{proof}
