...
In~\figref{fig:symInit}, given the unitialized set $\Lambda$ for
...
fresh location. This occurs when $\cfgnt{r}_f$ is not new or any other alias. (iii) a member of $\theta_\mathit{alias}$ restricts $\cfgnt{r}_f$ to
...
and $y$ of the same type. Note that when no constraint is specified in the graphs, then
...
The access of $x$ from $\cfgnt{l}_0$ creates a new input heap reference $\cfgnt{r}_1^i$ in the
summary heap shown in~\figref{fig:initHeap}(b). The reference $\cfgnt{r}_1^i$
points to the $\cfgnt{l}_\mathit{null}$ location under the constraint that
$\cfgnt{r}_1^i$ is null (\figref{fig:inintHeap}(d)): $\phi_{1a} := \cfgnt{r}_1^i = \cfgnt{r}_\mathit{null}$. The reference
$\cfgnt{r}_1^i$ points to the location $\cfgnt{l}_1$ under the constraint that $\cfgnt{r}_1^i$
is not $\mathit{null}$ ($\phi_{1b}$). The
location $\cfgnt{l}_1$, a fresh location of type $C$ (the type of field $\cfgnt{x}\rp$), is added to the heap. There are no aliases because the input heap was empty.
...
in~\figref{fig:initHeap}(d) contains the one potential aliases $\cfgnt{l}_1$ from the previous initialization for $\lp\cfgt{this}\  \cfgt{\$}\ \cfgnt{x}\rp$. It also shows the $\theta$-sets for this initialization and the resulting edge labels. Note that the ordering between aliases in $\theta_\mathit{alias}$ does not matter until a third field is initialized and there are two potential choices.
