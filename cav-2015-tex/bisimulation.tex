...
%This section establishes the correctness of
%the~\symtxt{}. Demonstrating correctness is important because many
%summary heap techniques are correct only when certain, often
%unstated, assumptions are made. To show the absence of such
%assumptions, it is necessary to demonstrate soundness and completess
%with respect to all properties provable by symbolic execution. The
%proof relies on the observation that a state in symbolic execution
%can be thought of as representing a set of possible concrete
%states. Similarly, a state in the~\symtxt{} machine represents a set
%of possible~\gsetxt{} states. The goal is to show that states in
%the~\symtxt{} machine behave exactly like the sets of~\gsetxt{}
...
Symbolic Execution (GSE). Intuitively, any properties proven with GSE
can also be proved using the symbolic heap algorithm.  GSE concretizes
the possible heap configurations along with various aliasing
relationships when an uninitialized field is dereferenced. The rules
for initializing and updating the fields in the Javalite semantics for
GSE are shown in the~\appref{app:gse}.

The proof shows the existence of a
bisimulation between sets of states related by GSE ($p \rgse
...
for GSE (see appendix): $\rgse = \rgse^\mathit{A} \cup
