...
\figref{fig:lazyInit}, \figref{fig:sym}, and \figref{fig:symInit}.
\end{definition}
...
\lemref{lem:succ} ensures at least one successor of $P$ and $q$. Let $p^\prime \in P^\prime$ be any
...
\begin{lemma}[$\rsum^*$ produces well-formed heaps]
\label{lem:S-determ}
...
\begin{lemma}[$\rightarrow_\mathit{FA}$ produces well-formed heaps]
\label{lem:FA-determ}
...
\begin{lemma}[$\rightarrow_\mathit{FW}$ produces well-formed heaps]
\label{lem:FW-determ}
...
\begin{lemma}[$\rcom$ produces well-formed heaps]
\label{lem:J-determ}
Given a state, $s_\sym$, with a deterministic heap,
$(\cfgnt{L}_\sym\ \cfgnt{R}_\sym) =
\mathrm{heap}(s_\sym)$, the new heap,
