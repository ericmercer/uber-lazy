...
$$
P \sqsubset Q \Longleftrightarrow \forall q \in Q\ (\forall p \in S\ (p \sqsubset q \Rightarrow p \in P))
$$
\end{definition}

\begin{definition}
A state relation $p \rightarrow_x q$ is extended to sets of states $P \hookrightarrow_x Q$ as
$$
P \hookrightarrow_x Q \Longleftrightarrow \forall q \in S\ (p \rightarrow_x q \Rightarrow q \in Q)
$$
The $\rgse$ relations over sets of states is The $\rsym$ relation extended to sets of states
\end{definition}
...
\forall p \in P\ (\forall p^\prime\ ( p \rgse p^\prime \Rightarrow \exists q^\prime\ ( (q \rsym q^\prime )\wedge (p^\prime\ \sqsubset\ q^\prime ))))
\end{equation}
\begin{equation}
\label{eqn:BisimulationBackwards}
\forall q \in Q\ (\forall q^\prime\ ( q \rsym q^\prime\Rightarrow \exists p^\prime\ ( (p \rgse p^\prime )\wedge (p^\prime\ \sqsubset\ q^\prime ))))
\end{equation}
