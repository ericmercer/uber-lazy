...
presented in \secref{sec:bisim}.

\textbf{Reference partitions}: new references are drawn from
  ...
  algorithm and generalized symbolic execution (see \secref{sec:bisim}).
\begin{compactitem}
...
\noindent\textbf{Type consistency}: all locations associated with a reference have the same type.
\[
\begin{array}{l}
\forall \cfgnt{r} \in \cfgnt{L}^\leftarrow\ (\forall (\phi\ \cfgnt{l}),(\phi^\prime\ \cfgnt{l}^\prime) \in \cfgnt{L}(r)\ (\\
...
central contribution of this work. The heap summary algorithm creates
many references that are auxiliary literals or stack literals to
define the structure of the bipartite graph.  It reserves input heap
reference for constraints expressing potential aliasing. A solver
