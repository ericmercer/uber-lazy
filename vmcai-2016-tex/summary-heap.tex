...
The path condition is a predicate of references in the heap. The environment is a partial function so $\eta(\cfgnt{x}) = \cfgnt{r}$ is the reference
...
omit the commas for compactness. The language only considers objects as values and does not support looping structures to focus the discussion on the symbolic heap representation. 
...
that do not update the symbolic heap are in 
