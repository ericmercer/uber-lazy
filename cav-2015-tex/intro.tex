...
%
%Two major factors behind the utility symbolic execution are of path conditions is that each one represents exactly the set of concrete program inputs that would result in a given execution path. For any given program, a concrete execution will follow the same path as a symbolic execution if and only if the concrete inputs satisfy the path condition. 

%Furthermore, because path conditions are logical predicates, they allow precise reasoning over potentially unbounded sets of program inputs. 
...
The primary virtue of symbolic execution is precision, being precise in at least two ways. First, the analysis it performs is sound and complete, capable of discovering any observable program behavior without returning any false positives. Second, the path condition is sound and complete, representing exactly the set of concrete program inputs that would result in a given execution path. 

However, symbolic execution's precision extends only as far as the capabilities capabilities of the underlying constraint solver. Thus, extending the capabilities of symbolic execution to reason about new theories is an area of active research. Of particular interest are symbolic dereferencing and updating operations~\cite{Qu:2011,Chen:2013}. Precisely modeling these operations is challenging, because it requires formulating logical predicates over an input domain containing a potentially unbounded number of references. 

Current strategies for symbolic dereferencing require making tradeoffs between the completeness of the analysis and the completeness of the path condition. For example, one simple approach is to terminate execution upon attempting to dereference any symbolic value~\cite{Clarke:2004}. While this preserves the integrity of the path condition, it comes at the cost of an incomplete analysis. Stepping towards a more complete analysis, dynamic symbolic execution (DSE), approximates the results of a symbolic dereferencing by substituting in a valid concrete value~\cite{Sen:2005,Godefroid:2005,Godefroid:POPL07,Tillmann:2008}. This allows a more complete analysis by allowing execution to continue, but the approximation renders the path condition incomplete~\cite{Elkarablieh:2009,Chen:2013}. Further progress in the analysis direction can be achieved by using value summary representations, where multiple values for a reference may be represented along a single execution path~\cite{Xie:2005,Yorsh:2008,Elkarablieh:2009,Dillig:2011,Torlak:2014,Sen:2014}. Systems using these representations perform analyses that are more complete than standard DSE methods, but their associated dereferencing operations require placing further restrictions on the input heap, such as limiting the number of nodes~\cite{Elkarablieh:2009}, the nature of aliasing relationships~\cite{Xie:2005,Babic:2007}, or the very shape of the heap itself~\cite{Dillig:2011}. Finally, at the opposite end of the spectrum lies generalized symbolic execution (GSE)~\cite{GSE03,KiasanKunit,Cadar:2008,Rosner:2015}, which accomplishes a fully complete analysis at the cost of an incomplete path condition. Thus, none of the current techniques provide the option of maintaining both a complete analysis and a complete path condition. This is in stark contrast to symbolic execution over primitive types, which enjoys both properties simultaneously. 

This paper introduces a technique that avoids these problems by preserving the following properties of classical symbolic execution: (i) Inputs are represented by unbounded and unconstrained input symbols. This lack of predetermined constraints is key to avoiding under-approximation, which in turn is required for performing a complete analysis. (ii) Points of nondeterminism are only present at control flow instructions, which is critical for maintaining a complete path condition for a given control flow path. 


%associated operations as completely as any other symbolic value. Using this technique, the symbolic input heap is truly a blank slate without any hidden restrictions, and the path condition captures the complete set of all heaps that follow a specific program path up to a given point of execution. To the knowledge of the authors, it
...
%configurations are created. This creates two problems: First, since testing each configuration requires following a separate
%execution path to it's termination, GSE induces a path explosion problem. Second, by splitting the 
%symbolic input space between the various aliasing choices, GSE effectively splits the path condition between 
%multiple symbolic states. Since subsequent reference 
%initializations are nondeterministic on the separated execution paths,
%it is not generally possible to merge path conditions once they have been split.
%By splitting paths without allowing the possibility of recombining them,
%GSE is both computationally expensive, and fails to produce
%complete path conditions.

%Attempt to argue for completeness:
%In searching for a solution to the path explosion problem, it is tempting to consider one of the several techniques developed for representing multiple heap shapes within a single predicate. However, these techniques all impose constraints on the symbolic input heap. 
%
...
%
%It is important to note that these constraints are not merely arbitrary parameters chosen for the purposes of preventing infinite recursion. Instead, they are fundamental limitations of the underlying representations. For example, methods that require setting bounds on the size of the input heap are often predicated on a memory model that requires fixing the address space from the beginning of program execution. 
%
%Since these constraints are built in to the core of these techniques, it is not obvious how to remove them. In the case of the fixed address space models, symbolic pointer dereferencing is accomplished by first conducting a search for the set of all potential aliases of compatible type within the address space, then producing an expression based on that set of aliases. Since number of terms in the expression depends on the size of the address space, it seems that dereferencing a pointer in a space with an unbounded number of aliases would require an expression with an unbounded number of terms.
%
%In all cases, these constraints impose hidden qualifications on the validity of the path condition. The path condition is only valid for the set of heaps satisfying the hidden constraints, instead of the complete set of all possible input heaps. The end results are that the symbolic execution may fail to discover valid program behavior, and the validity of any subsequent analysis relying on the exactness of the path condition will be compromised. 


% You can solve path explosion by bounding the execution and rolling 
%everything into one big equation, but then you’re incomplete.\
...
%\subsection{A Definitive Symbolic Heap}
%It is apparent that despite the many proposed solutions, none of them 
%represent the heap as an unconstrained, unbounded, and complete
%symbolic value. In other words, there is not yet a definitive symbolic heap. 
