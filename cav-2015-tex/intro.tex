...
\subsection{Heaps as Symbolic Values}
A significant body of research is focused on reasoning about symbolic input references in an unknown input heap. Of particular interest are symbolic dereferencing and updating operations. However, despite many years of intensive research, a definitive solution has remained elusive. To date, none of the proposed techniques has been able to preserve all of symbolic execution's most essential properties, in particular: 1) The ability reason over arbitrary, unbounded, and otherwise unconstrained symbolic input values, and 2) The ability to produce a path condition that exactly represents all possible program values for a given execution path. The most important techniques and their limitations are broadly defined below, while a more detailed evaluation of individual contributions is reserved for the related works section.
...
This paper introduces a technique for modeling the symbolic input heap, symbolic references, and their
