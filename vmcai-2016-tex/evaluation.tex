...
%\subsection{Analysis}
Although the utility of value-set representations has been demonstrated
in previous works,
it remains an open question whether the dereferencing scheme
presented here represents a practical alternative to the
case-splitting methods currently in use. It is possible 
that the benefits of path-reduction are offset by the increased
complexity of the underlying constraints problem. 
...
The results show that \symtst{} dramatically reduces 
the number of paths that are explored, without producing a commensurate
increase in constraint solver time. Performance improvement ranges from $4.8x$ for
