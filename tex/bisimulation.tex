...
$$
The definition embodies the notion of splitting groups of heaps at
certain operations. The set $P$ at a field access after base reference is known, for example, has two
potential successors: a non-null and null reference. The $\rsgse$ relation has two successors at that point to divide $P$ into the two outcomes. In contrast, the
...
$q^\prime$ such that $q \rsym q^\prime$, the proof posits that with any 
$p^\prime$ such that $p^\prime \sqsubset q^\prime$, it is possible to use the
