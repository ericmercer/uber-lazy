...
\end{mathpar}
\end{center}
\caption{Initialization for generalized symbolic execution, $s ::= \lp\cfgnt{L}\ \cfgnt{R}\ \cfgnt{r}\ \cfgnt{f} \cfgnt{C}\rp$, with $s \rinit^* s^\prime = s \rinit \cdots \rinit s^\prime \rinit s^\prime$.}
\label{fig:lazyInit}
\end{figure*}
