...
of concretization may cause the analysis to miss valid program behaviors. 
...
An alternative to concretization is to construct the heap in a lazy manner,
...
configurations are created, exacerbating the path explosion problem.
...
However, by placing an arbitrary bounds on the size of the input heap, the 
analysis performed by these techniques is not complete.
