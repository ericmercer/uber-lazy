...
The summary heap algorithm is defined using
Javalite~\cite{saints-MS}. The small-step semantics of Javalite,
defined as a syntactic machine using a CESK architecture, are modified
to perform symbolic execution on a summary heap. The surface (input)
and machine (state) syntax is shown
...
A summary heap is a bipartite graph on references, $\cfgnt{r}$, and
...
associated with the given location-field pair in the heap. References
in the graph are immutable. Locations, however, are mutated on
updates. The structure with the immutability of references mean that
look-ups in the heap are non-recursive and updates do not modify the
current path condition.

The soundness and completeness proofs rely on a set of invariants that
...
\item[Null and Uninitialized] means that every summary heap contains 
...
A key insight to the summary heap is that although many auxiliary or
stack literals are created in symbolic execution with lazy
initialization, only input heap references appear in constraints
expressing potential aliasing relationships. A solver reasons over the
aliasing relationships, and a satisfying assignment uniquely
identifies active edges in the graph. Those active edges describe a
representative concrete heap and its shape. For example, if the solver
assigns input heap variables $\cfgnt{r}$ and $\cfgnt{r}^\prime$ such
that $\cfgnt{r} = \cfgnt{r}^\prime$ is true, then there exists some
location $\cfgnt{l}$ and constraints $\phi$ and $\phi^\prime$ such
that (i) $\lp\phi\ \cfgnt{l}\rp \in \cfgnt{L}(\cfgnt{r})$; (ii)
...
Notice that references for objects from the new-expression are auxiliary literals
so they do not appear in any constraint in the summary heap as an aliasing option.

\begin{comment}
MOVE TO THE PROOF SECTION
To show the heap summary algorithm is sound and complete with respect
to properties proved by GSE, a transition relation that
relates states according GSE is defined as $\rgse$ or $p \rgse
p^\prime$. Similarly the transition relation for the new summary heap algorithm is given as
$\rsym$ or $q \rsym q^\prime$ (defined in the next section). For simplicity, both use lazy
initialization as opposed to lazier or lazier\#. The full operational semantics for $\rgse$ are in the appendix. 
\end{comment}
